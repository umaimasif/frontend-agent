...
"""
            ...
            # route entry
            route_path = "/" if p.lower() == "home" else f"/{p.lower()}"
        ...
        if navbar:
            ...
            if navbar_color.startswith("#"):
                ...
                # treat navbar_color as Tailwind class
                nav_elem = dedent(f"""
...
with st.sidebar:
    ...
    # Step 5: Advanced or not
        ...
        if st.session_state.qa_advanced:
    ...
    # Advanced questions (only if chosen)
        ...
        theme_choice = st.selectbox("10) Theme", ["light", "dark", "custom"], index=0 if answers.get("theme","light")=="light" else (1 if answers.get("theme")=="dark" else 2))
